...
export default function EditDesaPage() {
  ...
  const { id: village_id } = useParams() as { id?: string }
  const isEdit = Boolean(village_id)
  ...
  useEffect(() => {
    if (!isEdit || !village_id) return
    const fetchVillage = async () => {
      setLoading(true)
      try {
        const res = await fetch(`${baseUrl}/api/village/${village_id}`, {
          credentials: "include",
        })
        ...
        console.error("fetch village error", e)
        setError("Gagal mengambil data desa untuk diedit")
      } finally {
        setLoading(false)
      }
    }
    fetchVillage()
  }, [isEdit, village_id, baseUrl])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const url = isEdit && village_id ? `${baseUrl}/api/village/${village_id}` : `${baseUrl}/api/village`
      ...
      if (!res.ok) {
        ...
        throw new Error(txt || (isEdit ? "Gagal mengupdate desa" : "Gagal membuat desa"))
      }

      alert(isEdit ? "Desa berhasil diperbarui" : "Desa berhasil dibuat")
